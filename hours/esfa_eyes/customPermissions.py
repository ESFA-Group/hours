...
class hasGlobalSalesAccess(permissions.BasePermission):
    def has_permission(self, request, view):
        return bool(request.user and (request.user.is_superuser or request.user.is_global_sales_editor or request.user.is_global_sales_viewer))

class hasDetailedSalesAccess(permissions.BasePermission):
    def has_permission(self, request, view):
        return bool(request.user and (request.user.is_superuser or request.user.is_detailed_sales_editor or request.user.is_detailed_sales_viewer))
